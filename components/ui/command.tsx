...
export function CommandDialogBackdrop({
  ...
  return (
    <CommandDialogPrimitive.Backdrop
      className={cn(
        "fixed inset-0 z-52 bg-black/32 backdrop-blur-sm transition-all duration-200 data-ending-style:opacity-0 data-starting-style:opacity-0",
        className,
      )}
...
export function CommandDialogViewport({
  ...
  return (
    <CommandDialogPrimitive.Viewport
      className={cn(
        "fixed inset-0 z-52 flex flex-col items-center px-4 py-[max(--spacing(4),4vh)] sm:py-[10vh]",
        className,
      )}
...
export function CommandPanel({
  ...
  return (
    <div
      className={cn(
        "relative flex flex-1 min-h-0 flex-col overflow-hidden -mx-px not-has-[+[data-slot=command-footer]]:-mb-px rounded-md not-has-[+[data-slot=command-footer]]:rounded-md border border-b-0 bg-popover bg-clip-padding shadow-xs/5 [clip-path:inset(0_1px)] not-has-[+[data-slot=command-footer]]:[clip-path:inset(0_1px_1px_1px_round_0_0_calc(var(--radius-2xl)-1px)_calc(var(--radius-2xl)-1px))] before:pointer-events-none before:absolute before:inset-0 before:rounded-md **:data-[slot=scroll-area-scrollbar]:mt-2",
        className,
      )}
      {...props}
    />
...
export function CommandFooter({
  ...
  return (
    <div
      className={cn(
        "flex shrink-0 items-center justify-between gap-2 rounded-md border-t px-5 py-3 text-xs",
        className,
      )}
